...
public class WM_InOutModelFactory implements IModelFactory {
	@Override 	public Class<?> getClass(String tableName) {
		 if (tableName.equals(MWM_InOut.Table_Name)){
			 return MWM_InOut.class;
		 }
  		return null;
	}
	@Override	public PO getPO(String tableName, int Record_ID, String trxName) {
		 if (tableName.equals(MWM_InOut.Table_Name)) {
		     return new MWM_InOut(Env.getCtx(), Record_ID, trxName);
		 }
  		return null;
	}
	@Override	public PO getPO(String tableName, ResultSet rs, String trxName) {
		 if (tableName.equals(MWM_InOut.Table_Name)) {
		     return new MWM_InOut(Env.getCtx(), rs, trxName);
		   }
 		 return null;
	}
}
