...
public class DocumentEngine implements DocAction
{
	...
	public boolean processIt (String action)
	{
		...
		if (ACTION_Post.equals(m_action))
			return postIt(); 
		if (ACTION_WaitConfirmation.equals(m_action))
			return waitConfirmation();
		if (ACTION_WaitingPayment.equals(m_action))
			return waitPayment();
		if (ACTION_Drafted.equals(m_action))
			return draftIt();
		//
		return false;
	}	//	processDocument
	...
	public boolean waitPayment() {
		m_document.waitPayment();
		return true;
	}

	public boolean waitConfirmation() {
		m_document.waitConfirmation();
		return true;
	}
	...
	{
		return m_document.invalidateIt();  
	}	//	invalidateIt
	...
	{
		return m_document.reverseCorrectIt();			 
	...
	{ 
		return m_document.reverseAccrualIt();  
	}	//	reverseAccrualIt
	...
	{ 
		return m_document.reActivateIt();  
	}	//	reActivateIt
	...
	public String[] getActionOptions()
	{
		...
		if (isInProgress() || isApproved())
			return new String[] {ACTION_Complete, ACTION_WaitConfirmation,
