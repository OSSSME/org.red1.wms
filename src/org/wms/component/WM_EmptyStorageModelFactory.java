...
public class WM_EmptyStorageModelFactory implements IModelFactory {
	@Override 	public Class<?> getClass(String tableName) {
		 if (tableName.equals(MWM_EmptyStorage.Table_Name)){
			 return MWM_EmptyStorage.class;
		 }
  		return null;
	}
	@Override	public PO getPO(String tableName, int Record_ID, String trxName) {
		 if (tableName.equals(MWM_EmptyStorage.Table_Name)) {
		     return new MWM_EmptyStorage(Env.getCtx(), Record_ID, trxName);
		 }
  		return null;
	}
	@Override	public PO getPO(String tableName, ResultSet rs, String trxName) {
		 if (tableName.equals(MWM_EmptyStorage.Table_Name)) {
		     return new MWM_EmptyStorage(Env.getCtx(), rs, trxName);
		   }
 		 return null;
	}
}
