...
public class WM_MigrationModelFactory implements IModelFactory {
	@Override 	public Class<?> getClass(String tableName) {
		 if (tableName.equals(MWM_Migration.Table_Name)){
			 return MWM_Migration.class;
		 }
  		return null;
	}
	@Override	public PO getPO(String tableName, int Record_ID, String trxName) {
		 if (tableName.equals(MWM_Migration.Table_Name)) {
		     return new MWM_Migration(Env.getCtx(), Record_ID, trxName);
		 }
  		return null;
	}
	@Override	public PO getPO(String tableName, ResultSet rs, String trxName) {
		 if (tableName.equals(MWM_Migration.Table_Name)) {
		     return new MWM_Migration(Env.getCtx(), rs, trxName);
		   }
 		 return null;
	}
}
